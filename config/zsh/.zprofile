...
function set_server_color(){
    grep -q -e "SERVER_COLOR=[0-9]" /home/$USER/.config/zsh/.zprofile.local &> /dev/null || python -c "import socket; color = sum([ord(ch) for ch in socket.gethostname()])%255; print(f'SERVER_COLOR={color+100 if color < 100 else color}')" >> /home/$USER/.config/zsh/.zprofile.local
}
set_server_color
